package dev.tanuki.feature.auth.data.crypto

/**
 * Minimal pure-Kotlin SHA-256 so PKCE code-challenge generation is identical on Android
 * and iOS with no platform crypto dependency. Returns the 32-byte digest.
 */
internal fun sha256(message: ByteArray): ByteArray {
    ...
    val padded = message.copyOf(paddedLen)
    padded[message.size] = 0x80.toByte()
    for (i in 0 until 8) {
        padded[paddedLen - 1 - i] = (originalLenBits ushr (8 * i)).toByte()
    }
    ...
    intArrayOf(h0, h1, h2, h3, h4, h5, h6, h7).forEachIndexed { idx, value ->
        out[idx * 4] = (value ushr 24).toByte()
        out[idx * 4 + 1] = (value ushr 16).toByte()
        out[idx * 4 + 2] = (value ushr 8).toByte()
        out[idx * 4 + 3] = value.toByte()
    }
    return out
}
